...
class Database{
  ...
  async initSelf(){
    return new Promise((resolve, reject)=>{
      ...
      if(this.API_Toten){
        ...
        fetch('https://cloud.seatable.cn/api/v2.1/dtable/app-access-token/', msg)
  ...
  async getTablesHeader(){
    return new Promise((resolve, reject)=>{
      if(this.API_Toten && this.databaseAccessToken && this.databaseServerAddr && this.databaseUUID){
        const msg = {
          method: 'GET',
          headers: {
            accept: 'application/json',
        ...
        fetch(this.databaseServerAddr + 'api/v1/dtables/' + this.databaseUUID + '/metadata/', msg)
          .then(response => response.json())
          .then(response => {
            // 返回表头信息的数组
            let arr=[];
  ...
  async addOneRow(rowDataObj, table_name){
    return new Promise((resolve, reject)=>{
      if(this.databaseAccessToken&&this.databaseServerAddr&&this.databaseUUID&&rowDataObj&&table_name){
        const msg = {
          method: 'POST',
          headers: {
            accept: 'application/json',
        ...
        fetch(this.databaseServerAddr + 'api/v1/dtables/'+ this.databaseUUID +'/rows/', msg)
          .then(response => response.json())
          .then(response => {
  ...
  async selectRows(sql){
    return new Promise((resolve, reject)=>{
      if(this.databaseAccessToken&&this.databseAddr&&this.databaseUUID&&sql){
          // SQL查找数据
          const msg = {
            method: 'POST',
            headers: {
            ...
            // body: JSON.stringify({ convert_keys: true, sql: `SELECT * FROM UsersPassword WHERE 用户名 = 'lisi' AND  密码= '222';` })
  ...
  async updateOneRow(newRowDataObj, rowID, table_name){
    return new Promise((resolve, reject)=>{
      if(this.databaseAccessToken&&this.databaseServerAddr&&this.databaseUUID&&newRowDataObj&&rowID&&table_name){
        const msg = {
          method: 'PUT',
          headers: {
            accept: 'application/json',
  ...
  async getTableAllRows(table_name){
    return new Promise((resolve, reject)=>{
      if(this.databaseAccessToken&&this.databaseServerAddr&&this.databaseUUID&&table_name){
        const options = {
          method: 'GET',
          headers: {
            accept: 'application/json',
        ...
        fetch(this.databaseServerAddr + 'api/v1/dtables/' + this.databaseUUID + '/rows/?table_name='+table_name, options)
          .then(response => response.json())
          .then(response => {
...
//   let sql = `SELECT * FROM Table1 WHERE name = 'lisi' AND  password = '222';` ;
//   let res = await database.selectRows(sql);
//   // console.log(res);

//   let table_name = "Table1";
//   let id_tochange = "DoBx_PasSU-AFyB7NzoSlQ";
//   let newRowDataObj = {
//     name:"lisi_new",
//     password:"222_new",
//     test:"555_new"
//   };
//   console.log("=====================");
//   let res2 = await database.getTableAllRows(table_name);
//   console.log(res2);
// }
